...
public class ReserveDAO {
	...
	public String reserveTimeRealData(int ftno)
	  ...
	  public void reserveOk(ReserveVO vo)
	  {
		  ...
		  {
			  conn=CreateConnection.getConnection();
			  String sql="INSERT INTO gg_foodreserve_4 VALUES("
	  ...
	  public List<ReserveVO> reserveMyPageData(String id)
	  {
		  ...
		  {
			  conn=CreateConnection.getConnection();
			  String sql="SELECT r.fno,?,rno,rdate,rtime,inwon,regdate,reserve_no,name,poster,tel,type,menu,parking,address,ok "
	  ...
	  public ReserveVO mypageReserveInfo(int rno)
	  {
		  ...
		  try
		  {
			  ...
			  String sql="SELECT rno,rdate,rtime,reserve_no,inwon,name,score,poster,address,tel,type,parking "
					    +"FROM gg_foodreserve_4 r,gg_locationfood f "
					    +"WHERE r.fno=f.fno "
					    +"AND rno=?";
			  ps=conn.prepareStatement(sql);
			  ps.setInt(1, rno);
